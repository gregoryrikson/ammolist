class Comfy::Cms::ContentController < Comfy::Cms::BaseController
  ...
  def feed
    @layout = Comfy::Cms::Layout.find_by_label('Post')
    @pages = @cms_site.pages.unscope(:order).published.shared.where(:layout_id => @layout.id).order('comfy_cms_pages.updated_at')
    render action: 'feed.rss.builder'
  end

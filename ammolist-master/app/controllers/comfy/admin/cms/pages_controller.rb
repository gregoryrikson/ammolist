class Comfy::Admin::Cms::PagesController < Comfy::Admin::Cms::BaseController
  ...
  def create
    @page.save!
    post_to_media if params[:page][:is_shared].to_s == "1"
...
protected
  ...
  def post_to_media
    # facebook
    @fb = @site.shared_media.find_by_name("Facebook")
    if (@fb)
      connection = @fb.shared_media_keys.first
      time1 = Time.new
      if connection
        @api = Koala::Facebook::API.new(connection.value)
        @api.put_wall_post("#{@page.label} #{@page.url} #{time1.to_s}")
      end
    end

    # twitter
    @twitter = @site.shared_media.find_by_name("Twitter")
    if (@twitter)
      connection = @twitter.shared_media_keys.find_all
      if connection
        client = Twitter::REST::Client.new do |config|
          connection.each do |conn|
          case conn.name
            when "consumer_key"
              config.consumer_key = conn.value
            when "consumer_secret"
              config.consumer_secret = conn.value
            when "access_token"
              config.access_token = conn.value
            when "access_token_secret"
              config.access_token_secret = conn.value
            end
          end
        end
        client.update("#{@page.label} #{@page.url}")
      end
    end
    
    # for google_plus use dlvr.it & rss feed

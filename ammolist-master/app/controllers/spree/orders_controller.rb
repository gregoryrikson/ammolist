  ...
  class OrdersController < Spree::StoreController
    ...
    def populate
      ...
      if params[:quantity].is_a?(Hash)
        quantity_not_null = params[:quantity].reject{|k, v| v==0 || v==""}
        if quantity_not_null.length > 0
          quantity_not_null.each do |k, v|
            if populator.populate(params[:variant_id][k], params[:quantity][k])

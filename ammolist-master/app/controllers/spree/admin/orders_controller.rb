  ...
  module Admin
    class OrdersController < Spree::Admin::BaseController
      ...
      def update_shipment_upgrade
        shipment_upgrades = params[:dataSend].to_a
        shipment_upgrades.each do |shipment_upgrade|
          shipment_upgrade = shipment_upgrade[1]
          @shipment = Spree::Shipment.find_by_id(shipment_upgrade[:shipment_id])
          @shipment.shipment_upgrades.update(shipment_upgrade[:shipment_upgrade_value], selected: shipment_upgrade[:shipment_upgrade_checked])
          @shipment.order.updater.update_shipment_total
        end
        if  @shipment.order.updater.persist_totals
          response_msg = [ :status => 'success', :message => "Selected Shipping upgrade has been added.", :order_total => @shipment.order.total, :display_total => @shipment.order.display_total]
        else

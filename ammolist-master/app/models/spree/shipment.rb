...
module Spree
  class Shipment < Spree::Base
    ...
    has_many :shipment_upgrades, -> { order('id ASC') }, dependent: :delete_all
    has_many :shipping_method_upgrades, through: :shipment_upgrades
    ...
    def final_price
      cost_with_upgrade + adjustment_total
    end
    ...
    def shipping_constraint
      ...
      Spree::ConstraintType.all.each do |constraint|
        ...
        manifest.each do |item|
          total = total + item.quantity * item.variant["#{constraint.variant_column_name}"]
        end
    ...
    def disallow_message(shipping_rates)
      message = ""
      shipping_constraint.each do |constraint| 
        allowance = shipping_rates.shipping_allowance(constraint.total, constraint)
        if allowance
          message = allowance.map{|k,v| v }.join(' ,')
        end
      end
      message
    end
    ...
    def shipment_upgrades_options
      shipment_upgrades_opt = []
      shipping_method_id = selected_shipping_rate.try(:shipping_method_id)
      unless shipping_method_id.nil? || shipping_method_id == 0
        selected_shipping_method_upgrades = Spree::ShippingMethodUpgrade.where(shipping_method_id: shipping_method_id)
        unless selected_shipping_method_upgrades.empty?
          shipment_upgrades_opt = shipment_upgrades.where(shipping_method_upgrade: selected_shipping_method_upgrades)
          if shipment_upgrades_opt.empty?
            Spree::ShippingMethodUpgrade.find_each do |smu|
              self.shipment_upgrades.create(:shipping_method_upgrade_id => smu.id,
                                             :selected => false)
            end
            shipment_upgrades_opt = shipment_upgrades.where(shipping_method_upgrade: selected_shipping_method_upgrades)
          end
    ...
    def update_shipment_upgrades_status(hash_id)
      shipment_upgrades.each do |s|
        if hash_id.include?(s.id)
          shipment_upgrades.update(s.id, selected: true)
        else
          shipment_upgrades.update(s.id, selected: false)
        end
      end
    end

    def selected_shipment_upgrade
      self.shipment_upgrades_options.where(selected: true)
    end

    def upgrade_total
      self.selected_shipment_upgrade.to_a.sum(&:cost)
    end

    def selected_shipment_upgrade_id=(hash_id)
      shipment_upgrades.each do |s|
        if hash_id.include?(s.id)
          shipment_upgrades.update(s.id, selected: true)
        else

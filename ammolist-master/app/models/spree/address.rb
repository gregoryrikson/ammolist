module Spree
  class Address < Spree::Base
    ...
    validates :firstname, :lastname, :address1, :city, :country, presence: true, if: :require_data?
    validates :zipcode, presence: true, if: :require_zipcode? && :require_data?
    validates :phone, presence: true, if: :require_phone? && :require_data?

    validate :state_validate, :postal_code_validate, if: :require_data?

    alias_attribute :first_name, :firstname
    alias_attribute :last_name, :lastname

    attr_accessor :require_data 

    def self.build_default
      country = Spree::Country.find(Spree::Config[:default_country_id]) rescue Spree::Country.first
      new(country: country, require_data: true)
    end

    def self.build_default_require_data(is_require_data)
      country = Spree::Country.find(Spree::Config[:default_country_id]) rescue Spree::Country.first
      new(country: country, require_data: is_require_data)
    end
